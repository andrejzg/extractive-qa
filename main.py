...
def run_experiment(
    ...
    **unused
    ...
    loss_per_example_t = tf.reduce_sum(divergence_t * span_mask_t, axis=1)
    loss_t = tf.reduce_sum(loss_per_example_t)  # is the mean valid with a mask? (differing # of examples)
    tf.summary.scalar('sum_train_loss', loss_t)
